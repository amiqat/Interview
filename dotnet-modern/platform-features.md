...
.NET 8 brings several tangible improvements worth the upgrade cost. On the performance side, Native AOT enables ahead-of-time compilation for faster startup and smaller container images, and the Request Delegate Generator source-generates minimal API delegates to remove runtime reflection overhead. New APIs like frozen collections (`FrozenDictionary<K,V>`, `FrozenSet<T>`) optimise read-heavy lookups, and `TimeProvider` introduces a testable time abstraction replacing hard-to-mock `DateTime.UtcNow` calls. The framework adds `IExceptionHandler` for cleaner global exception handling and keyed services in DI for injecting different implementations by key. `System.Text.Json` gains better source generator support and performance. The upgrade also brings long-term support (LTS) status, meaning three years of security patches compared to .NET 6's end-of-life timeline.
